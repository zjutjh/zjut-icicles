...
class PoseCtrlArm:
    ...
    def hand_threading(self, lmList):
        if self.event.is_set():
            ...
            if sum(fingers) == 5: self.media_ros.pub_vel(0.3, 0)
            if sum(fingers) == 0: self.media_ros.pub_vel(-0.3, 0)
            if sum(fingers) == 1 and fingers[1] == 1: self.media_ros.pub_vel(0, 0.3, 0)
            if sum(fingers) == 2 and fingers[1] == 1 and fingers[2] == 1: self.media_ros.pub_vel(0, -0.3, 0)
            if sum(fingers) == 3 and fingers[0] == 0 and fingers[4] == 0: self.media_ros.pub_vel(0, 0, 1.5)
            if sum(fingers) == 4 and fingers[0] == 0: self.media_ros.pub_vel(0, 0, -1.5)
        ...
        if self.event.is_set():
            ...
            if len(pointArray) != 0:
                ...
                if angle11 < -37:
                    if self.index >= 30:
                        self.media_ros.RobotBuzzer()
                        self.media_ros.pub_arm(joints, runtime=1000)
                        self.car_status = True
                    else:self.index += 1
                else:
