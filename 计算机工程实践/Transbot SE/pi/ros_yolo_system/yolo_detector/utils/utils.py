...
def get_random_data_with_Mosaic(annotation_line, input_shape, max_boxes=50, hue=.1, sat=1.5, val=1.5):
    ...
    for line in annotation_line:
        ...
        if len(line_content) > 1:
            ...
            yolo_boxes = _read_yolo_labels(image_path)
            box = np.array(yolo_boxes, dtype='int32') if len(yolo_boxes) > 0 else np.zeros((0, 5), dtype='int32')
        
        # 是否翻转图片
        flip = False;#关闭以防止左右转混淆
...
def _read_yolo_labels(image_path):
    """从与 image_path 同名的 .txt 读取 YOLO 格式标签并转为 [x_min,y_min,x_max,y_max,cls] 绝对坐标。若无标签档则返回空 list。"""
    ...
    for l in lines:
        ...
        if len(parts) != 5:
            continue
        cls_id, cx, cy, w_rel, h_rel = map(float, parts)
        x_min = int((cx - w_rel / 2) * iw)
        y_min = int((cy - h_rel / 2) * ih)
        x_max = int((cx + w_rel / 2) * iw)
        y_max = int((cy + h_rel / 2) * ih)
        # 边界修正
        x_min = max(0, x_min)
        y_min = max(0, y_min)
        x_max = min(iw, x_max)
        y_max = min(ih, y_max)
        boxes.append([x_min, y_min, x_max, y_max, int(cls_id)])
    return boxes
